...
void basic_tests_1() {
	...
	if(verbose) printf("Base1 created directly with Derived *.\n");
        {
	...
	if(verbose) printf("Base1 assigned from SharedPtr<Derived>.\n");
        {
            SharedPtr<Base1> sp2;
            {
    ...
    {
	if(verbose) printf("Test Copy constructor and template copy constructor\n");
        {
            SharedPtr<Derived> sp(new Derived);
    ...
    if(verbose) printf("Test Assignment operator and template assignment operator\n");
    {
        {
            SharedPtr<Derived> sp(new Derived);
    ...
    if(verbose) printf("Test static_pointer_cast\n");
    {
        SharedPtr<Derived> sp(new Derived);
        SharedPtr<Base1> sp2(sp);
    ...
    if(verbose) printf("Test dynamic_pointer_cast\n");
    {
    ...
    if(verbose) printf("Test to make sure works with MI\n");
    {
        SharedPtr<Base2> sp2;
        {
            SharedPtr<Base1> sp1;
    ...
    if(verbose) printf("Test const assignment\n");
    {
